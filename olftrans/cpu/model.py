...
class Model:
    ...
    def __init__(self, num=1, floatType=np.double, **states_or_params):
        ...
        for key, val in states_or_params.items():
            if key in self.states:
                ...
                raise ValueError(f"Argument '{key}' with value '{val}' is neither state nor params.")
        ...
        for key, val in _states.items():
            if isinstance(val, Iterable):
                assert all([isinstance(v, Number) for v in val]), \
                ...
                raise TypeError(f"State '{key}' has value that is neither iterable nor number {val}")


        _params = copy.deepcopy(self.params)
        for key, val in _params.items():
            if isinstance(val, Iterable):
                assert (len(val)==num), f"Param '{key}' is iterable of length {len(val)} but num = {num}"
            elif isinstance(val, Number):
                self.params[key] = np.full((num,), val, dtype=self._float)
            else:
                raise TypeError(f"Param '{key}' has value that is neither iterable nor number {val}")
    ...
    def record(self, steps, states):
        ...
        for key in states:
            ...
            self.recorder[key] = np.zeros((steps, self.num), dtype=self.states[key].dtype)
class OTP(Model):
    States = dict(
        v=(0.,0, 1e9),
        I=0.,
    ...
    def gradient(self, stimulus=0.):
        d_x1 = self.br*self.v*(1.-self.x1) - self.dr*self.x1
        d_x2 = self.a2*self.x1*(1.-self.x2) - self.b2*self.x2 - self.kappa*np.cbrt(self.x2*self.x2) * np.cbrt(self.x3*self.x3)
        d_x3 = self.a3*self.x2 - self.b3*self.x3
        d_uh = self.duh
        d_duh = -2*self.a1*self.b1*self.duh + self.a1*self.a1*(stimulus-self.uh)
...
class NoisyConnorStevens(Model):
    ...
    def gradient(self, stimulus=0.):
        ...
        alpha[_mask] = 0.1
        alpha[~_mask] = -0.01*(self.v[~_mask]+50.+self.ns[~_mask])/alpha[~_mask]
        ...
        _mask = np.abs(alpha) <= 1e-7
        alpha[_mask] = 1.
        alpha[~_mask] = -.1*(self.v[~_mask]+35.+self.ms[~_mask])/alpha[~_mask]
    ...
    def non_gradient(self):
        spike = (self.v1 <= self.v2) * (self.v <= self.v2) * (self.v2 > -30.)
        v1 = self.v2
        v2 = self.v
        spike = (spike > 0.) * (self.refactory >= 0)
        refactory = self.refactory - (spike > 0.) * self.refperiod
        return dict(spike=spike, v1=v1, v2=v2, refactory=refactory)
    
    
def compute_fi(NeuronModel, Is, repeat=1, input_var='stimulus', spike_var='spike', voltage_var='v', dur=2., start=.5, dt=1e-5, verbose=True, neuron_params=None) -> tp.Tuple[tp.Tuple[np.ndarray, np.ndarray], Model]:
    neuron_params = neuron_params or {}
    Is = np.atleast_1d(Is)
    N_amps = len(Is)
    N_comps = repeat*N_amps
    wav = np.ascontiguousarray(utils.generate_stimulus('step', dt, dur, (start, dur), Is).T)  # shape (NT, NComp)
