...
class Data:
    ...
    def setup(self, clean=False):
        ...
        if clean:
            ...
            while confirm.lower() not in ['y', 'n', 'yes', 'no']:
                confirm = input(f"WARNING: setup(clean) with remove everything in cache folder, proceed? [n]")
                if confirm == '':
                    # assume as n
                    break
        ...
        for key, DClass in zip(['REST','FI','OTP'], [DataRest, DataFI, DataOTP]):
            ...
            if df is None or df.empty or not os.path.exists(self.fnames[key]):
                columns = ['Time', 'FileName'] + \
                    ...
                    [f'metadata.{k}' for k in DataMetadata.__dataclass_fields__.keys()]
                df = pd.DataFrame(columns=columns)
                df.to_json(self.fnames[key], orient='index')

            # convert list and tuple to numpy array
            df = df.applymap(lambda x: np.asarray(x) if isinstance(x, (list, tuple)) else x)
            self.dfs[key] = df

    def save(self, data_type:tp.Union["otp","rest","fi"], data, metadata, fname=None):
        if data_type.upper() not in ['FI', 'REST', 'OTP']:
            raise ValueError(f"data_type {data_type} not understood, must be OTP/REST/FI")
    ...
    def find(self, data_type: tp.Union["otp","rest","fi"], key: str, loc_args: tp.Callable = None):
        if data_type.upper() not in ['FI', 'REST', 'OTP']:
            raise ValueError(f"data_type {data_type} not understood, must be OTP/REST/FI")
        data_type = data_type.upper()
