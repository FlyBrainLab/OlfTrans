...
from olftrans import KERNEL_DIR

class OTP(NDComponent):
    accesses = ['conc'] # rate
    updates = ['I']  # current
    params = ['br', 'gamma', 'dr', 'c1',
              'a2', 'b2', 'a3', 'b3',
              'k23', 'CCR', 'ICR', 'L', 'W']
    internals = OrderedDict([('v', 0.),
                             ('uh', 0.),
                             ('duh', 0.),
                             ('x1', 0.),
                             ('x2', 0.),
    ...
    def get_update_template(self, float_type):
        with open(os.path.join(KERNEL_DIR, 'OTP.cu'), 'r') as f:
    ...
    def get_update_func(self, dtypes):
        from pycuda.compiler import SourceModule
        mod = SourceModule(self.get_update_template(self.dtype),
                           options=self.compile_options)
        func = mod.get_function("OTP")

        type_dict = {k: dtype_to_ctype(dtypes[k]) for k in dtypes}
        type_dict.update({'fletter': 'f' if type_dict['param_' + self.params[0]] == 'float' else ''})
