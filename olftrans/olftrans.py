...
def estimate_sigma(resting_spike_rate, cache=True):
    ...
    return np.interp(
        x=resting_spike_rate,
        xp=NoisyConnorStevens.DATA.rest_f,
        fp=NoisyConnorStevens.DATA.rest_sigma,
    )
...
def estimate_current(spike_rate, sigma, sigma_atol=1e-4, sigma_rtol=1e-4, cache=True):
    ...
    if abs(sigma_err) > sigma_atol or sigma_rerr > sigma_rtol:
        warn(
            f"Required Sigma value too different from cached Sigma, recomputing F-I curve..."
        )
...
def estimate_affinity(amplitude, steady_state_current, cache=True):
    ...
    bvd = (BR * v / DR).ravel()
    idx = np.argsort(bvd)
    max_current = np.percentile(
        OTP.DATA.ss, 99
    )  # clip current value at 99 percentile of available values
    steady_state_current = np.clip(steady_state_current, 0.0, max_current)
...
def estimate_dissociation(
    ...
    if peak_current is None:
        ...
        intp = interp2d(
            OTP.DATA.br * OTP.DATA.amplitude, OTP.DATA.dr, OTP.DATA.peak, kind="linear"
        )
        drs = np.linspace(1e-2, 1000.0, 500)
...
def estimate_resting_spike_rate(sigma, cache=True):
    ...
    return np.interp(
        x=sigma,
        xp=NoisyConnorStevens.DATA.rest_sigma,
        fp=NoisyConnorStevens.DATA.rest_f,
    )
